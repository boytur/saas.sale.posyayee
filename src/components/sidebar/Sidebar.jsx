...
const Sidebar = ({ isSidebarOpen }) => {
  ...
  const getActiveButton = (route) => {
    return location.pathname === route
      ? "bg-[#E4E3FF] text-[#4C49ED] rounded-md border-[#4C49ED]"
      : "";
  };
  ...
  return (
    // sidebar for desktop
    <div className={`bg-white text-black h-full w-[65%] hidden 
      ${!isSidebarOpen ? "md:w-[13rem]" : "md:w-[3.6rem]"} lg:block lg:relative border z-50 transition-width duration-300 ease-in-out`}>

      <div className="w-full px-1 py-2 flex flex-col gap-2 transition-all duration-300 ease-in-out">
        <Link
          to="/"
          className={`h-[3rem] rounded-lg flex ${isSidebarOpen && "justify-center"} items-center gap-2 px-2 hover:bg-[#E4E3FF] hover:text-[#4C49ED] hover:border-[#4C49ED] ${getActiveButton(
            "/"
          )}`}
        >
          <div>
